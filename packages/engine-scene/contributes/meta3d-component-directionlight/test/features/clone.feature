Feature: Clone
	As a Clone
	I want to clone directionLight
	So that I can get cloned one

	Background: get contribute and create a state
		Given I get contribute
		And create a state

	Scenario: clone specific count of directionLights
		Given create a directionLight
		When clone 2 directionLights
		Then get 2 cloned directionLights

	Scenario: set cloned directionLight's color by source directionLight's color
		Given create a directionLight
		And set the directionLight's color to c1
		When clone 2 directionLights
		Then get 2 cloned directionLights' color should return c1, c1

	Scenario: set cloned directionLight's intensity by source directionLight's intensity
		Given create a directionLight
		And set the directionLight's intensity to i1
		When clone 2 directionLights
		Then get 2 cloned directionLights' intensity should return i1, i1