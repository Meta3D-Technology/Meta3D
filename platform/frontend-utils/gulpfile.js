...
gulp.task("updatePatchVersion", function (done) {
    let versionFilePath = "./src/config/VersionConfig.res"
    ...
    fs.writeFileSync(versionFilePath, content.replace(/^let\sgetPlatformVersion\s\=\s\(\)\s\=>\s.+$/img, "let getPlatformVersion = () => " + "\"" + semver.inc(currentVersion, "patch") + "\"")
    )
