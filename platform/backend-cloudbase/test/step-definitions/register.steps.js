...
const feature = jest_cucumber_1.loadFeature("./test/features/register.feature");
jest_cucumber_1.defineFeature(feature, test => {
    ...
    test('register', ({ given, when, then, and }) => {
        ...
        given('prepare funcs', () => {
            ...
            addDataFunc.returns(PromiseTool_1.resolve(null));
        });
        when('register', () => {
            return RegisterService_1._register(addDataFunc, username, password);
        });
    ...
    test('check username fail', ({ given, and, when, then }) => {
        ...
        and('register', () => {
            notHasDataFunc.withArgs("user", { username: username }).returns(most_1.just(false));
        });
        when('check username', () => {
        });
        then('should fail', () => {
            return RegisterService_1._checkUserName(notHasDataFunc, username).observe(result => {
    ...
    test('check username success', ({ given, and, when, then }) => {
        ...
        given('prepare funcs', () => {
            _createFuncs(sandbox);
            notHasDataFunc.withArgs("user", { username: username }).returns(most_1.just(true));
        });
        when('check username', () => {
        });
        then('should success', () => {
