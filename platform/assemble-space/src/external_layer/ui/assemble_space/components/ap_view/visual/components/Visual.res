...
module Method = {
  let useSelector = ({allCanvasData}: FrontendUtils.ApViewStoreType.state) => {
    allCanvasData
  }
}
...
let make = (~service: service) => {
  let allCanvasData = ReduxUtils.ApView.useSelector(service.react.useSelector, Method.useSelector)
