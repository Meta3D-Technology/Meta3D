...
let feature = loadFeature("./test/features/contributeInspector.feature")
...
defineFeature(feature, test => {
  ...
  test(."show nothing", ({given, \"when", \"and", then}) => {
    ...
    then("should show nothing", () => {
      ContributeInspectorTool.buildUI(
        ~sandbox,
    ...
    \"and"("set a1's new name to new1", () => {
      useSelectorStub :=
        createEmptyStub(refJsObjToSandbox(sandbox.contents))->returns(
          (
            id->Some,
            list{SelectedContributesTool.buildSelectedContribute(~id, ~newName="new1"->Some, ())},
          ),
          _,
        )
    })
