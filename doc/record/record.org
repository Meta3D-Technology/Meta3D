...
** TODO add geometry, pbrMaterial components(include worker)
...
** TODO refactor: GameContributeType remove 'state, 'gameObject, 'transform, 'config
...
** TODO refactor: geometry: setXxx(e.g. VerticesUtils) not return state(because it's mutable operation)


** TODO refactor: geometry, pbrMaterial->dispose: remove duplicate




** TODO rename needDisposedXxxArray, disposedXxxArray to Xxxs


** TODO refactor: remove component_protocols -> state?
