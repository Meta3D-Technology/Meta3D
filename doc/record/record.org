...
** TODO refactor(work_plugins): change function from let xxx to function xxx

** TODO refactor(work_plugins): remove Type.ts for work plugin

** TODO refactor(work_plugins): Utils add setState

** TODO refactor(work_plugin_protocols): type states use workPluginName as key
export type states = { [workPluginName]: state, }

** TODO refactor(work_plugins): webgl1 worker->main, render: job should add postfix;
export type states = { [workPluginName]: state, }
...
** TODO pass render run test:clone cube
