...
api = {
    engineWholePackage service(actually engine-whole-protocol),
}
...
meta3d-engine-scene/scene view render/game view render/web render/asset/ui/event/basic-protocol

# two kind render package implement:
# SceneViewThreeRender
# GameViewThreeRender

one editor engine whole package implement = Scene + SceneViewThreeRender(new api: createSpecificDefaultGameObjects) + GameViewThreeRender + Asset
...
new api: getPackageService(protocol)=>nullable<package service>
...
if(stop){
    getPackageService(protocol)->nullable<package service>->renderOnce
}
...
one publish engine whole package implement = Scene  + WebThreeRender + Asset
