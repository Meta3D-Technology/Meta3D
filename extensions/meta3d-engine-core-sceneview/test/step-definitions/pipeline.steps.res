...
defineFeature(feature, test => {
  ...
  let _buildPipelineContribute = (
    ...
    ~allPipelineData=[],
    (),
  ): Meta3dEngineCoreSceneviewProtocol.PipelineManagerType.pipelineContributeForRegister => {
    pipelineName,
    createStateFunc,
    initFunc,
    getExecFunc,
  ...
  test(."restore pipeline state", ({given, \"when", \"and", then}) => {
    ...
    \"and"(
      "change ps1 to ps2",
      () => {
        (PipelineTool.getPipelineState(s2.contents, pipelineName)->Obj.magic)["data1"]
    ...
    given(
      ...
      () => {
        ...
        let job2 = (
          ...
          getStatesFunc(. meta3dState)->setStatesFunc(meta3dState, _)->Meta3dBsMost.Most.just
        }
