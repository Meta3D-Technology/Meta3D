...
): Meta3dEngineCoreSceneviewProtocol.RegisterPipelineType.jobOrders => {
  jobOrders->Meta3dCommonlib.ArraySt.map((
    {pipelineName, insertElementName, insertAction} as jobOrder,
  ): Meta3dEngineCoreSceneviewProtocol.RegisterPipelineType.jobOrder => {
    pipelineName,
    insertElementName,
    insertAction: switch insertAction {
    | #before => Meta3dEngineCoreSceneviewProtocol.RegisterPipelineType.Before
    | #after => Meta3dEngineCoreSceneviewProtocol.RegisterPipelineType.After
    },
  })
}
...
let deepCopy = (api: Meta3dType.Index.api, extensionProtocolName, meta3dState) => {
  let state: Meta3dEngineCoreSceneviewProtocol.StateType.state = api.getExtensionState(.
    meta3dState,
    extensionProtocolName,
  )
