...
defineFeature(feature, test => {
  ...
  test(."init pipelines", ({given, \"when", \"and", then}) => {
    ...
    \"when"("register pipeline1 contribute with config1", () => {
  ...
  test(."test register two pipelines that pipeline has one job", ({given, \"when", \"and", then}) => {
    let state1 = ref(Obj.magic(1))
    let state2 = ref(Obj.magic(1))
  ...
  test(."test register two pipelines that pipeline has two jobs", ({given, \"when", \"and", then}) => {
    let state1 = ref(Obj.magic(1))
    let state2 = ref(Obj.magic(1))
    let meta3dState: ref<Meta3dType.Index.state> = ref(Obj.magic(1))
  ...
  test(."test register three pipelines with init, update pipeline jobs", ({
    ...
    given("register pipeline2 contribute with one update pipeline job", () => {
      stub2 := createEmptyStubWithJsObjSandbox(sandbox)
      let job1 = (
        meta3dState,
        {getStatesFunc, setStatesFunc}: Meta3dEngineCoreProtocol.StateType.operateStatesFuncs,
    ...
    given("register pipeline3 contribute with one init pipeline job", () => {
