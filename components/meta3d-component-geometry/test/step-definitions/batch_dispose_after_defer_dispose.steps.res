...
defineFeature(feature, test => {
  ...
  test(."if not dispose geometry from all gameObjects, not dispose geometry\'s data", ({
    ...
    \"and"(%re("/^set geometry's vertices to v(\d+)$/")->Obj.magic, () => {
      state :=
    ...
    \"and"(%re("/^add the geometry to g(\d+)$/")->Obj.magic, () => {
      state :=
    ...
    given(%re("/^defer dispose the geometry from g(\d+)$/")->Obj.magic, () => {
      state :=
    ...
    then(%re("/^get the geometry's vertices should return v(\d+)$/")->Obj.magic, () => {
