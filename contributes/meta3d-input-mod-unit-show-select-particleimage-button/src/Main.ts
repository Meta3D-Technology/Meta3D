...
export let getContribute: getContributeMeta3D<inputContribute<data>> = (api) => {
    return {
        inputName: "ModUnitShowSelectParticleImageButtonInput",
        func: (meta3dState, [emitterTypeFieldName]) => {
            return Promise.resolve(
                api.nullable.getWithDefault(
                    api.nullable.map(state => state[emitterTypeFieldName] == emitterType.Particle, api.action.getActionState<any>(meta3dState, actionName)),
                    false
                )
            )
        }
