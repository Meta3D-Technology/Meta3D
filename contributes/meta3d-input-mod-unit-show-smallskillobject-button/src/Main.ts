...
export let getContribute: getContributeMeta3D<inputContribute<data>> = (api) => {
    return {
        inputName: "ModUnitShowSmallSkillObjectButtonInput",
        func: (meta3dState) => {
            return Promise.resolve(
                api.nullable.getWithDefault(
                    api.nullable.map(state => state.hasSmallSkillObject, api.action.getActionState<state>(meta3dState, actionName)),
                    false
                )
            )
        }
