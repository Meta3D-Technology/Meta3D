            ...
            return new Promise((resolve, reject) => {
                resolve(eventSourcingService.on<inputData>(meta3dState, eventName, 0, (meta3dState,) => {
                    ...
                    if (allSelectedCareerFeatureData.filter(d => d.positive).count() >= 3) {
                        api.message.warn(api.action.getActionState<languageState>(meta3dState, languageActionName).language == language.Chinese ? "最多只能选择3个正面职业特征" : "You can only select up to 3 positive career features")
