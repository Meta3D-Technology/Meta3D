...
import { actionName, state, uiData, base64 } from "meta3d-action-mod-unit-upload-particle-image-protocol"
import { eventName, inputData } from "meta3d-action-mod-unit-upload-particle-image-protocol/src/EventType"
...
let _loadImage = (api: api): Promise<base64> => {
...
export let getContribute: getContributeMeta3D<actionContribute<uiData, state>> = (api) => {
    return {
        ...
        init: (meta3dState) => {
            ...
            return new Promise((resolve, reject) => {
                resolve(eventSourcingService.on<inputData>(meta3dState, eventName, 0, (meta3dState, uiData, imageData, [selectedSkillObjectEmitterParticleImageIndexFieldName]) => {
                    ...
                    meta3dState = api.action.setActionState<state>(meta3dState, actionName, {
                        ...state,
                        images: state.images.set(selectedSkillObjectEmitterParticleImageIndexFieldName, api.nullable.return(imageData))
                    })
        ...
        handler: (meta3dState, uiData, actionParams) => {
            return _loadImage(api).then((imageData) => {
                if (!imageData) {
                    return meta3dState
                }
        ...
        createState: () => {
            return {
                images: api.immutable.createMap()
            }
        }
    }
}
