...
export let getContribute: getContributeMeta3D<inputContribute<data>> = (api) => {
    return {
        inputName: "ModUnitArmorTypeInput",
        func: (meta3dState) => {
            let allModelData = api.nullable.getWithDefault(
                api.nullable.map(state => [
                    getLanguageTextData(api, meta3dState, state.languageTextData, languageKey.Light),
                    getLanguageTextData(api, meta3dState, state.languageTextData, languageKey.Heavy),
                    getLanguageTextData(api, meta3dState, state.languageTextData, languageKey.Mech),
