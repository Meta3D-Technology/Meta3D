...
import { componentName, geometry, dataName, vertices, indices } from "meta3d-component-geometry-worker-protocol";

export let execFunc: execFuncType = (engineCoreState, { getStatesFunc, setStatesFunc }) => {
	let states = getStatesFunc<states>(engineCoreState)
	let { mostService, webgl1Service, engineCoreService, immutableService, gl, vbo, allGeometryIndices } = getState(states)
	...
	return mostService.callFunc(() => {
		...
		// let [newVerticesVBOMap, newIndicesVBOMap] = initGeometryUtils(engineCoreState, [webgl1Service, engineCoreService],


		// 	gl, verticesVBOMap, indicesVBOMap, allGeometryIndices,)
