...
import { addBasicCameraView, addGeometry, addPBRMaterial, addPerspectiveCameraProjection, addTransform, cloneGameObject, createGameObject, disposeGameObjects, disposeGameObjectTransformComponent, getAllGameObjects, getGeometry, getTransform } from "engine-facade/src/GameObjectAPI"
...
import { createPBRMaterial, getAllPBRMaterials, setDiffuseColor } from "engine-facade/src/PBRMaterialAPI"
...
import { componentName as transformComponentName, dataName as transformDataName, transform } from "meta3d-component-transform-protocol"
...
import { clonedGameObjects } from "../../extension_protocols/meta3d-engine-core-protocol/src/contribute_points/scene_graph/GameObjectContributeType"
        ...
        engineCoreState =
            registerWorkPlugin(
                ...
                [
                    {
                        pipelineName: "update",
                        insertElementName: "send_render_data_webgl1_worker_main_engine",
                        insertAction: "after"
                    }
                ]
            )
        ...
        engineCoreState =
            registerWorkPlugin(
                ...
                getWebGL1RenderWorkPluginContribute({
                    mostService, webgl1Service, engineCoreService, immutableService, workPluginWhichHasAllRenderComponentsName: webgl1WorkPluginName,
                    transformData: {
                        componentName: transformComponentName,
...
function _createCubeGameObject(engineCoreState: engineCoreState, engineCoreService: engineCoreService): [engineCoreState, gameObject] {
...
function _createClonedGameObjects(engineCoreState: engineCoreState, engineCoreService: engineCoreService, sourceGameObject: gameObject): [engineCoreState, clonedGameObjects] {
    ...
    let tran1 = getTransform(engineCoreState, engineCoreService, clonedGameObjects[0][0])

    engineCoreState = setLocalPosition(engineCoreState, engineCoreService, tran1, [5, 0, 0])

    let tran2 = getTransform(engineCoreState, engineCoreService, clonedGameObjects[0][1])

    engineCoreState = setLocalPosition(engineCoreState, engineCoreService, tran2, [-5, 0, 0])

    return [engineCoreState, clonedGameObjects]
}

function _createScene(engineCoreState: engineCoreState, engineCoreService: engineCoreService, canvas: HTMLCanvasElement) {
    let data = _createCubeGameObject(engineCoreState, engineCoreService)
    engineCoreState = data[0]
    let gameObject = data[1]

    let data2 = _createClonedGameObjects(engineCoreState, engineCoreService, gameObject)
    engineCoreState = data2[0]
...
function _loop(meta3dState: meta3dState, frameIndex: number) {
    if (frameIndex === 20) {
        let engineCoreState = getExtensionState<engineCoreState>(meta3dState, _getEngineCoreExtensionName())
    ...
    updateEngine(meta3dState, _getEngineCoreExtensionName()).then((meta3dState) => {
        renderEngine(meta3dState, _getEngineCoreExtensionName()).then((meta3dState) => {
            requestAnimationFrame(() => {
                _loop(meta3dState, frameIndex + 1)
            })
        })
    })
}
...
_init(useWorker).then((meta3dState) => _loop(meta3dState, 0))
