...
// V13：demo 改用 snapshot_EliteGiantess10 的 Tripo 模型（1efef564），
// 它与 Mixamo 官方 lod2 绑骨（model_EliteGiantess10_lod2.fbx）是同一套网格 →
// 转换时传入 lod2 官方骨架作为 rest pose 参照（officialRestPose），
// 转换后骨骼 rest transform 与 Mixamo 官方完全一致（D6 验收）。
const MODEL_URL = '/snapshot/tripo_convert_1efef564-f149-410e-8df7-aff0deb624f6/tripo_convert_1efef564-f149-410e-8df7-aff0deb624f6.fbx';
/** Mixamo 官方 lod2 绑骨骨架（rest pose 对齐参照） */
const OFFICIAL_REST_URL = '/snapshot/model_EliteGiantess10_lod2.fbx';
const ANIM_URL = '/asset-lib/unit-action/src/asset/action/elitegiantess/default/DeathHeadshot/1.fbx'; // TEMP-V13: DeathHeadshot test (revert to Idle)

// ===== 按钮 =====
const btnLoadModel = document.getElementById('btn-load-model') as HTMLButtonElement;
const btnLoadOfficial = document.getElementById('btn-load-official') as HTMLButtonElement;
const btnConvert = document.getElementById('btn-convert') as HTMLButtonElement;
const btnLoadAnim = document.getElementById('btn-load-anim') as HTMLButtonElement;
...
let animSkeleton: THREE.Object3D | null = null;
/** V12：Mixamo 官方 lod2 骨架（rest pose 对齐参照），加载模型时一并异步加载 */
let officialRestRoot: THREE.Object3D | null = null;

// window 访问出口：demo 专用（e2e 通过 window.__xxx__ 断言），统一走 win 保持简洁
const win = window as any;

/** e2e 探针：计算 model 世界包围盒（含骨骼变形，供「模型在视口内」断言） */
function computeModelBBox(): { min: number[]; max: number[]; center: number[]; size: number[] } | null {
    ...
    model.traverse((node) => {
        ...
        if (mesh.isSkinnedMesh && mesh.geometry) {
            ...
            box.union(b);
        } else if ((node as THREE.Mesh).isMesh && (node as THREE.Mesh).geometry) {
            const geo = (node as THREE.Mesh).geometry as THREE.BufferGeometry;
            if (geo.boundingBox === null) geo.computeBoundingBox();
            if (!geo.boundingBox) return;
            const b = geo.boundingBox.clone();
            b.applyMatrix4((node as THREE.Mesh).matrixWorld);
            box.union(b);
        }
    });
...
function computeBoneStats(): { count: number; nanCount: number; infCount: number; zeroScaleCount: number; sample: string[] } | null {
    if (!model) return null;
    let nanCount = 0;
    let infCount = 0;
    ...
    model.traverse((node) => {
        ...
        for (let i = 0; i < 16; i++) {
            ...
            if (Number.isNaN(v)) nanCount += 1;
            else if (!Number.isFinite(v)) infCount += 1;
        }
        const ws = new THREE.Vector3();
        bone.getWorldScale(ws);
        if (ws.lengthSq() === 0 || !Number.isFinite(ws.x + ws.y + ws.z)) zeroScaleCount += 1;
        if (count <= 5) sample.push(`${bone.name}@(${bone.position.x.toFixed(3)},${bone.position.y.toFixed(3)},${bone.position.z.toFixed(3)})`);
    });
...
win.__SET_CAMERA__ = (pos: number[], target: number[]) => {
...
// ===== 工具函数 =====
function log(message: string, level: 'info' | 'ok' | 'warn' | 'error' = 'info'): void {
...
function collectBoneNames(root: THREE.Object3D): string[] {
    const names: string[] = [];
    root.traverse((node) => {
        if ((node as THREE.Bone).isBone) names.push(node.name);
    });
    return names;
}

/** 收集所有动画轨道名 */
function collectTrackNames(clip: THREE.AnimationClip): string[] {
    return clip.tracks.map((t) => t.name);
}

/** 动画轨道匹配统计（D5）：供面板显示 + e2e 断言 */
// demo 文件私有类型，e2e 通过 window.__ANIM_MATCH__ 访问
interface AnimMatchStat {
    total: number;
    matched: number;
    /** 匹配率 0~1 */
    rate: number;
 ...
 */
function computeAnimMatch(): AnimMatchStat | null {
    if (!model || !animClip) {
    ...
    const stat: AnimMatchStat = { total, matched, rate, clipName: animClip.name };
...
function loadOfficialModel(): void {
    const t0 = performance.now();
...
function fitCameraToModel(root: THREE.Object3D): void {
...
function collectSkinnedMeshCount(root: THREE.Object3D): number {
    let count = 0;
    root.traverse((node) => {
        if ((node as THREE.SkinnedMesh).isSkinnedMesh) count += 1;
    });
    return count;
}

/** 探针：Root 的 localScale / worldScale（方案 P2-1） */
function probeRootScale(root: THREE.Object3D): void {
    root.updateWorldMatrix(true, true);
    ...
    log(
        localScale.lengthSq() === 3 ? '[探针] Root 缩放为单位阵' : '[探针] ⚠️ Root 携带非单位缩放，注意 attach() 烘焙',
        localScale.lengthSq() === 3 ? 'ok' : 'warn',
    );
}

/** 探针：Twist 骨的实际 parent（方案 P2-4） */
function probeTwistBones(bones: string[], root: THREE.Object3D): void {
    ...
    root.traverse((node) => {
        if ((node as THREE.Bone).isBone && twistPattern.test(node.name)) {
...
function setupMixer(): void {
    if (!model || !animClip) return;
